      ...
      <section className="mx-auto max-w-container px-4 pt-8 sm:px-6">
        ...
        <div className="flex gap-4 overflow-hidden">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="skeleton aspect-[16/10] h-48 w-64 shrink-0 rounded-xl" />
          ))}
        </div>
      </section>
    );
  ...
  return (
    <section className="group/rail mx-auto max-w-container px-4 pt-8 sm:px-6 relative">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-ink sm:text-2xl">From the newsroom</h2>
          <p className="text-xs text-ink-mute">Articles, insights, and media coverage from the OBS network.</p>
        </div>
        <button
      ...
      <div className="relative">
        ...
        >
          {articles.map((a) => (
            <div key={a._id} className="w-[260px] sm:w-[300px] shrink-0 snap-start">
